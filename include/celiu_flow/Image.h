...
#include "celiu_flow/Stochastic.h"
...
public:
	...
	Image(const T& value,int _width,int _height,int _nchannels);
	Image(const Image<T>& other);
	~Image(void);
	virtual Image<T>& operator=(const Image<T>& other); //deep copy
	 ...
	 * pixel access
	 */
	T& operator () (const uint32_t r, const uint32_t c) {return pData[r * imWidth + c];}
	T operator () (const uint32_t r, const uint32_t c) const {return pData[r * imWidth + c];}
	T& operator () (const uint32_t r, const uint32_t c, const uint32_t k) {return pData[(r * imWidth + c) * nChannels + k];}
	T operator () (const uint32_t r, const uint32_t c, const uint32_t k) const {return pData[(r * imWidth + c) * nChannels + k];}
	...
	//if real-valued, clamp to [0, 1]; else clamp to [0, 255]
	void clampToNormalRange();
...
void Image<T>::desaturate(Image<T1> &image) const
{
	...
	T1* data=image.data();
	/*
	 * EVH: I've found this colorType isn't getting set; for now, ignore it at this one place where it's used
	 */
#if 0
	if(colorType == RGB)
	{
		cout << "rgb " << imWidth << " x " << imHeight << endl;
		for(int i=0;i<nPixels;i++)
		{
	...
	{
		cout << "non-rgb(" << colorType << ") " << imWidth << " x " << imHeight << endl;
#endif
		for(int i=0;i<nPixels;i++)
		{
			const int offset=i*3;
...
{
	ASSERT_ALWAYS(image1.matchDimension(image2));
	ASSERT_ALWAYS(image2.matchDimension(image3));
...
{
	ASSERT_ALWAYS(matchDimension(image1));
	const T1*& pData1=image1.data();
	for(int i=0;i<nElements;i++)
		pData[i]+=pData1[i];
}
...
template <class T>
void Image<T>::clampToNormalRange()
{
	T ImgMax;
	if(IsFloat())
		ImgMax = 1;
